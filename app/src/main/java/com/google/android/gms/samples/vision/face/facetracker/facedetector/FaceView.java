package com.google.android.gms.samples.vision.face.facetracker.facedetector;
...
public class FaceView extends View {
    ...
    private void drawFaceAnnotations(Canvas canvas, double scale) {
        ...
        for (int i = 0; i < mFaces.size(); ++i) {
            ...
            float left = (float) scale * (x - xOffset);
            float top = (float) scale * (y - yOffset);
            float right = (float) scale * (x + xOffset);
            float bottom = (float) scale * (y + yOffset);
//            float scale = 2560.0f / 960.0;
            Log.d("MyLog1", left + " " + top + " " + right + " " + bottom );
            Log.d("MyLog2", face.getPosition().x + " " + face.getPosition().y + " "
